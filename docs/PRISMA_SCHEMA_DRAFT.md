...
This is a starter Prisma schema for Codex. It is intentionally explicit rather than minimal. Codex may split, refine or adjust naming, but must preserve the model decisions in `docs/DATABASE_FINAL_MODEL.md`.
...
model TeamCoach {
  ...
  photoMedia MediaAsset? @relation("TeamCoachPhoto", fields: [photoMediaId], references: [id])
...
- This schema may need small Prisma relation-name fixes during implementation. Preserve the domain decisions even if syntax is adjusted.
- Use `BigInt` consistently because rr-management uses `bigserial` IDs and we want safe source ID mapping.
