...
export function AdminTopbar({ navItems }: AdminTopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[rgba(255,255,255,0.1)] bg-[rgba(7,22,41,0.78)] backdrop-blur-xl">
      <div className="flex flex-col gap-2 px-4 py-3 sm:gap-3 sm:px-6 sm:py-3.5 lg:px-8">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-1.5 sm:space-y-2">
            <p className="rr-kicker text-[color:var(--rr-gold)]">Backoffice deportivo</p>
            <h1 className="text-[1.18rem] font-semibold text-white sm:text-[1.5rem]">
              {currentLabel}
            </h1>

            <div className="flex flex-wrap items-center gap-1.5 text-[0.78rem] text-[color:var(--rr-muted)] sm:gap-2 sm:text-[0.82rem]">
              {activeTeam ? (
                <span className="inline-flex min-h-8 items-center rounded-full border border-[rgba(243,203,69,0.3)] bg-[rgba(243,203,69,0.1)] px-2.5 font-medium text-[color:var(--rr-gold)] sm:px-3">
                  Equipo: {formatSlugLabel(activeTeam)}
                </span>
              ) : null}
              {activeSeason ? (
                <span className="hidden min-h-8 items-center rounded-full border border-white/10 bg-white/5 px-3 font-medium md:inline-flex">
          ...
          <div className="flex w-full items-center gap-2 sm:w-auto sm:flex-row sm:items-center lg:items-start">
            <Link
              href="/"
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 text-[0.9rem] font-semibold text-[color:var(--rr-muted)] transition hover:border-[rgba(243,203,69,0.28)] hover:bg-[rgba(243,203,69,0.08)] hover:text-white sm:min-h-10 sm:w-auto sm:gap-2 sm:px-4"
            >
