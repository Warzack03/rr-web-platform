...
export function PlayerDerivedMetrics({
  ...
  const items = getPlayerDerivedMetrics(playerType, stats, statsLevel);

  return (
    <section
      className={
        teamType === "academy"
          ? "rr-panel-dark border-[color:var(--rr-border)] px-5 py-5 md:px-6"
          : "rr-panel border-[color:var(--rr-border-strong)] px-5 py-5 md:px-6"
      }
    >
      <div className="flex items-center gap-3">
        <span className="rr-kicker text-[0.82rem] text-[color:var(--rr-gold)]">
          Metricas derivadas
        </span>
      ...
      <div
        className={
          teamType === "academy"
            ...
            : "mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-1"
        }
      >
        {items.map((item) => (
          <PlayerStatTile
